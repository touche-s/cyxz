...
| 模块 | 端口 | 职责 | 评级 |
|---|---|---|---|
| cyxz-gateway | 8080 | 网关：路由、鉴权、信任头防伪造剥离、`/internal` 防外泄 | 良好 |
...
| cyxz-upload | 9006 | 上传：基础设施型 | 合理 |

### B 端治理服务（3 个，已建成）

| 模块 | 端口 | 职责 | 状态 |
|---|---|---|---|
| cyxz-governance | 9009 | 内容治理中心：举报受理与处置编排 | 已建 |
...
1. **RBAC 资源评估器**：`@circlePerm` 权限查询收归 auth 服务，经 `AuthPermissionPort` Feign 链路校验圈子权限
